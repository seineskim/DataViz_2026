...
\begin{enumerate}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.98\textwidth]{viz4_SK.pdf}
		\caption{Yes-vote share per year by national party (Top 10 NP, EP1).}
	\end{figure}
	
	\noindent
	I use the roll-call metadata file to extract the year of each roll-call vote and merge it onto the long-format vote data. 
	The date variable is stored as an Excel serial number, so I convert it to a calendar date using 
	\texttt{as.Date(., origin = "1899-12-30")} before extracting the year. I then compute average Yes shares by year and national party and display them in a bar plot (restricted to the top 10 national parties for readability). Some roll-calls have missing date information in the metadata; those are excluded from the year-based plots.
	...
	\medskip
	\noindent\textbf{Output (remaining missing years after merge).}
	\begin{Verbatim}[fontsize=\small]
...
[1] 9864
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{viz5_SK.pdf}
		\caption{Average Yes-vote share per year by EP Group (EP1).}
	\end{figure}
	
	\noindent
	The line plot shows how average Yes-vote rates change over time for each EP group. 
	While some groups display relatively stable voting patterns across years, others exhibit noticeable fluctuations, suggesting changes in voting behavior across roll calls.
	...
	\medskip
	\noindent\textbf{Output (first rows of EP-group-by-year averages).}
	\begin{Verbatim}[fontsize=\small]
		   year  `EP Group` avg_yes
		<chr> <chr>        <dbl>
		1 1979  C            0.421
		2 1979  E            0.491
		3 1979  G            0.265
